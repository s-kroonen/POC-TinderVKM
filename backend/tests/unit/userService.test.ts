...
describe("UserService", () => {
  ...
  it("calls repository.create with hashed password on register", async () => {
    mockRepo.create.mockResolvedValue({ _id: "1", email: "a@b.com", password: "hashedpw" } as any);
  ...
  it("finds a user by email", async () => {
    mockRepo.findByEmail.mockResolvedValue({ email: "user@test.com" } as any);
    const user = await service.findByEmail("user@test.com");
    expect(user?.email).toBe("user@test.com");
  });

  it("saves the user via repo.save", async () => {
    const user = { _id: "1", email: "x@y.com", liked: [], skipped: [] } as any;
